...
function App() {
  ...
  // Reset puzzle whenever the index changes
  useEffect(() => {
  ...
  const handleShowMove = () => {
    ...
    if (currentMoveIndex < 2) {
      // Show arrow only; don’t update boardPosition
      setCurrentMoveIndex((i) => i + 1);
    } else {
      // It’s time for the user to play
  ...
  const handleSquareClick = (square) => {
    ...
    if (selectedSquares.length === 0) {
      ...
      // Pause 1 second with both squares highlighted, then evaluate
      setTimeout(() => {
        setHighlightedSquares({});
  ...
  const evaluateUserMove = async (from, to) => {
    ...
    // Build the sequence: first two known moves, then the user's guess
    const sequence = [
    ...
    // Play out moves with arrows
    playMoveSequence(sequence, userGuess === correctMove);

    // If the move was incorrect, fetch an explanation from our serverless function
  ...
  const playMoveSequence = (moves, isCorrect) => {
    ...
    setTimeout(() => {
      if (isCorrect) {
        ...
        // Do nothing here, as the serverless call sets the message
      }
      // Clear arrows after showing final move
      setTimeout(() => setArrows([]), 700);
    }, moves.length * 1000 + 300);
  };

  const handleRevealSolution = () => {
    // Show the first three moves (including the correct 3rd move), then explanation
    const puzzle = puzzles[currentPuzzleIndex];
    const correctMove = puzzle.moves[2];
    const sequence = [puzzle.moves[0], puzzle.moves[1], correctMove];
